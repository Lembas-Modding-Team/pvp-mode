...
public class ServerConfigurationImpl extends AutoForgeConfigurationManager implements ServerConfiguration
{
    ...
    protected void onPropertyChanged (ConfigurationPropertyKey<?> key, Object oldValue, Object newValue)
    {
        ...
        else if (key.getInternalName ().equals (ACTIVE_COMBAT_LOGGING_HANDLERS.getInternalName ()))
        {
            ...
            if (!server.getCombatLogManager ().getActivatedHandlerNames ().equals (activeCombatLoggingHandlers))
            {
                new HashSet<> (server.getCombatLogManager ().getActivatedHandlerNames ())
                    .forEach (server.getCombatLogManager ()::deactivateHandler);

                if (!activeCombatLoggingHandlers.isEmpty ())
                {
                    activeCombatLoggingHandlers.forEach (server.getCombatLogManager ()::activateHandler);
                    logger.info ("Activated the following pvp combat logging handlers: %s",
